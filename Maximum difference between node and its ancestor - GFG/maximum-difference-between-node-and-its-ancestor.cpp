...
int findMaxDiff(Node* root, int &maxDiff)
{
    ...
    int leftAns = root->data - left;
    int rightAns = root->data - right;
    
    if(root->left && root->right)
    {
        maxDiff = max(maxDiff, max(leftAns, rightAns));
...
int maxDiff(Node* root)
{
    ...
    int ans = findMaxDiff(root, maxDiff);
